...
interface PaymentData {
  ...
  bookingType?: string; // "Pool Session", "Event", "Sports", etc.
  items: OrderItem[];
  total: number;
  ...
  duration?: string; // Event duration for conflict checking
...
export default function Payment() {
  ...
  useEffect(() => {
    const checkBookingAvailability = async () => {
      if (!paymentData || paymentData.orderType !== "booking") {
        setIsAvailable(true);
        return;
      }

      // Skip availability check for memberships and radio ads as they don't have specific time slots
      const isMembership = paymentData.bookingType?.includes("Membership") ||
        paymentData.bookingType?.includes("Package");

      if (isMembership || !paymentData.bookingDate) {
        setIsAvailable(true);
        return;
      }
      ...
      try {
        ...
        // For Pool Sessions, include the time in the date string (same format as Pool.tsx)
  ...
  const processingFee = paymentData ? paymentData.total * 0.02 : 0;
  const finalTotal = paymentData ? paymentData.total + processingFee : 0;

  // Initialize Paystack payment hook
  const initializePayment = usePaystackPayment({
    publicKey: PAYSTACK_PUBLIC_KEY,
    email: email || user?.email || "customer@example.com",
    amount: Math.round(finalTotal * 100), // Paystack uses smallest currency unit (pesewas)
    currency: "GHS",
    metadata: {
      custom_fields: [
        {
    ...
    try {
      ...
      if (paymentData?.orderType === "delivery") {
        ...
        await api.post("/orders/", {
          ...
          total_price: paymentData.total, // Record base price in database
          status: "Pending",
        });
        ...
        if (!isMembership && bookingDateStr) {
        ...
        // Always construct the date in the proper format: "Date at Time" 
        // Don't use bookingDetails as it contains extra information that breaks parsing
        const bookingDate = paymentData.bookingTime
          ? `${paymentData.bookingDate || ""} at ${paymentData.bookingTime}`
          : paymentData.bookingDate || "";

        try {
          const bookingPayload: any = {
            guest_name: paymentData.bookingName || "",
            email: paymentData.bookingEmail || email || "", // Pass the email for notifications
            booking_type: bookingType,
            status: "Pending", // Admin needs to verify payment before approving
          };

          // Only add date for non-membership bookings
          if (!isMembership && bookingDate) {
            bookingPayload.date = bookingDate;
          }

          if (bookingType.includes("Pool Session")) {
            bookingPayload.slots = requestedSlots;
          }

          // Add price from payment data (use base price for database)
          if (paymentData.total) {
            bookingPayload.price = paymentData.total;
          }
  ...
  if (!paymentData || (paymentData.orderType === "booking" && isCheckingAvailability && isAvailable === null)) {
    return (
      <Layout>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span>₵{paymentData.total.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                  ...
                  <Button
                    ...
                    disabled={
                      ...
                      !email.includes("@") ||
                      isCheckingAvailability ||
                      (paymentData?.orderType === "booking" && isAvailable === false)
                    }
                  >
                    {isProcessing ? (
                      <>
